...
for task_spec in gym.registry.values():
    if ("Unitree" in task_spec.id or "Piper" in task_spec.id) and "Isaac" not in task_spec.id:
...
# Expose package root there so fully-qualified paths like
# "unitree_rl_lab.tasks....PointNetActorCritic" can be resolved.
...
def _resolve_policy_class_for_rsl_eval(agent_cfg: RslRlOnPolicyRunnerCfg):
    """Resolve dotted policy class path for rsl-rl eval(class_name) compatibility."""
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    # rsl-rl uses eval(policy.class_name) in on_policy_runner module scope.
    # Convert dotted paths to imported symbols to avoid package attribute-chain issues.
